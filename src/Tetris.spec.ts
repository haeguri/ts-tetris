...
describe("Tetris", () => {
  ...
  describe("← 키를 누르면 블락을 왼쪽으로 이동시킨다.", () => {
    it("왼쪽에 자리가 있으면 블락의 cell, rotation pivot 위치가 왼쪽으로 1만큼 이동된다.", () => {});

    it("왼쪽에 자리가 없으면 블락을 이동시킬 수 없다.", () => {});
  });

  describe("→ 키를 누르면 블락을 오른쪽으로 이동시킨다.", () => {
    it("오른쪽에 자리가 있으면 블락의 cell, rotation pivot 위치가 오른쪽으로 1만큼 이동된다.", () => {});

    it("오른쪽에 자리가 없으면 블락은 이동되지 않는다.", () => {});
  });

  describe("↓ 키를 누르면 블락을 아래로 이동시킨다.", () => {
    it("아래에 자리가 있으면 블락의 cell, rotation pivot 위치가 아래로 1만큼 이동된다.", () => {});
  ...
  describe("↑ 키를 누르면 블락을 회전시킨다.", () => {
    ...
    it("왼쪽에는 회전할 자리가 없는 경우가 생기지 않는다.");

    it("오른쪽에 회전할 자리가 없으면 회전한 위치에서 '블락의 넓이 - 1' 횟수 만큼 이동을 시도하고 이동한다.", () => {});
